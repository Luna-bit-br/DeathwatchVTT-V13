...
async function evaluateAndSendRoll(formula, rollData, flavor){
  if(!formula){ return ui.notifications.error("Fórmula de dano inválida."); }
  const r = await new Roll(formula, rollData).evaluate({async:true});
  r.toMessage({speaker: ChatMessage.getSpeaker({actor: rollData.actor}), flavor: flavor});
}
